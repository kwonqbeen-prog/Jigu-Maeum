...
기후 위기로 인한 불안과 무력감을 매일 주어지는 일상 속 작은 실천으로 돌보는 **기후 불안 케어 서비스**입니다.
...
- 💚 **기후 불안에 특화된 감정 돌봄** — 기후 불안이라는 특수한 맥락과 사용자의 감정&성향에 최적화된 맞춤형 미션을 생성합니다.
...
체크인(감정/의욕/장소/자유 입력)과 사용자 프로필(대처 방식, 사회적 활동 선호, 관심 분야), 최근 14일 이력을 바탕으로 Upstage **Solar LLM**이 맞춤형 기후 행동 미션을 생성합니다. 오늘의 의욕 수준(낮음/보통/높음)에 따라 한 번에 받는 미션 개수도 1개 / 3개 / 5개로 달라집니다.

### 2) 마음 지구(Mind Planet) 성장 시각화 & 동기부여

누적 완료 미션 수에 따라 캐릭터 "마음 지구"가 막 태어난 지구 → 조용한 지구 → 깨어나는 지구 → 함께 걷는 지구 → 숨쉬는 지구 → 빛나는 지구, 6단계로 성장합니다. 연속 실천일(streak), 업적 배지와 함께 사용자의 꾸준한 실천을 시각적으로 보여주어 지속적인 동기를 부여합니다.
...
- **판단 기준**
  - 미션 개수: 오늘의 의욕(에너지 레벨: 낮음/보통/높음)에 따라 `MISSION_COUNT_BY_ENERGY`(`src/data/constants.js`) 기준으로 1개 / 3개 / 5개를 생성
  - 유형 균형: `carbon`(생활 속 탄소배출 감축) / `nature`(자연·야외 활동) / `social`(사회적 대화·모임) 중 최근 14일간 가장 적게 제시된 유형을, 미션이 여러 개일 때는 최소 1개 포함하고 1개일 때는 그 유형을 우선 고려
  ...
  - 사회적 배려: 사회적 활동을 기피하는 사용자에게는 미션이 2개 이상일 때 `social` 유형을 최대 1개로 제한하고, 난이도는 `light`로 고정
...
- 응답은 코드펜스 없는 순수 JSON만 반환하도록 강제 (`missions` 배열 길이가 오늘 요청한 미션 개수와 정확히 일치해야 함)

### 안정성 확보 로직 (`src/hooks/useMissionGeneration.js`)

- 생성된 미션 제목이 최근 이력과 자카드 유사도(문자 단위) 0.7 이상으로 겹치면, 재생성 사유를 프롬프트에 덧붙여 한 번 더 요청
- Solar API 호출이 실패하거나 형식이 어긋나면 로컬 폴백 미션 풀(`src/data/missionPool.js`, 유형당 9개)에서 요청한 개수만큼 부족한 유형 순으로 안전하게 대체 제공

### 마음 지구 성장 로직 (`src/components/common/PlanetOrb.jsx`, `src/data/achievementRules.js`)

- 누적 완료 미션 수를 6단계 임계값(0 / 1 / 3 / 10 / 25 / 50)과 비교해 마음 지구의 단계·이름·대륙 색상을 결정
